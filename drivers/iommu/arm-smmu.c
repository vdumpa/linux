...
static void __iomem *__arm_smmu_page(struct arm_smmu_device *smmu, int n)
{
...
{
	void __iomem *base = __arm_smmu_page(smmu, page);
					  ...
					  size_t granule, bool leaf, void *cookie)
{
	...
	if (stage1) {
		...
		if (cfg->fmt != ARM_SMMU_CTX_FMT_AARCH64) {
			...
			iova |= cfg->asid;
			do {
				arm_smmu_write_cb(smmu, cfg->cbndx, reg, iova);
				iova += granule;
			} while (size -= granule);
		} else {
			iova >>= 12;
			...
			do {
				arm_smmu_write_cb_q(smmu, cfg->cbndx, reg, iova);
				iova += granule >> 12;
			} while (size -= granule);
		}
	} else {
		...
		iova >>= 12;
		do {
			arm_smmu_write_cb_addr(smmu, cfg->cbndx, reg, iova);
			iova += granule >> 12;
		} while (size -= granule);
	}
}
					      ...
					      dma_addr_t iova)
{
	...
	reg = __arm_smmu_page(smmu, smmu->cb_base + idx) + ARM_SMMU_CB_ATSR;
